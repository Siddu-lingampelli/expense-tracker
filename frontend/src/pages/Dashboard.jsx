...
const Dashboard = () => {
  ...
  const doughnutData = data ? {
    ...
    datasets: [{
      ...
      backgroundColor: ['#16A34A', '#2563EB', '#D97706', '#7C3AED', '#0891B2', '#DB2777', '#65A30D', '#0D9488'],
      borderWidth: 0,
    }],
  } : null;
  ...
  if (error) {
    return <div className="text-xs text-destructive">Failed to load dashboard.</div>;
  }
        ...
        {data?.recentTransactions?.length > 0 ? (
          <div>
            {data.recentTransactions.map((t) => (
              <div key={t._id} className="flex items-center justify-between px-4 py-3 border-b border-border last:border-b-0 hover:bg-secondary/30 transition-colors">
