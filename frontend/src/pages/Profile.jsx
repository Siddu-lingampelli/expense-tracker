        ...
        {tabs.map((t) => (
          <button
            ...
            className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-1.5 text-xs font-medium rounded-sm transition-colors ${
              activeTab === t.id ? 'bg-primary text-primary-foreground' : 'text-secondary-foreground hover:text-foreground'
            }`}
          >
        ...
        <div className="border border-border rounded-md p-4">
          ...
          <form onSubmit={handleProfileSubmit} className="space-y-3">
            ...
            <div className="space-y-1">
              ...
              <select value={profileForm.currency} onChange={(e) => setProfileForm({ ...profileForm, currency: e.target.value })} className="block w-full rounded-md border border-input bg-background px-3 py-2 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary focus-visible:ring-offset-1">
                {currencies.map(c => <option key={c.code} value={c.code}>{c.code} &mdash; {c.name} ({c.symbol})</option>)}
              </select>
            </div>
            <div className="flex justify-end pt-2">
              <button type="submit" className="px-4 py-2 text-xs font-medium bg-primary text-primary-foreground rounded-md hover:opacity-90 active:scale-[0.97] transition-all duration-150 disabled:opacity-50">
